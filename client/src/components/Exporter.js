...
class Exporter extends React.Component {
	...
	openModal() {
		this.props.dispatch(
			exporterActions.updateExporter({ exporterModalOpen: true })
		);
		if (this.props.exporter.exportComplete === 100) {
			this.props.dispatch(
				exporterActions.updateExporter({ exportComplete: 0 })
			);
		}
	}
	closeModal() {
		this.props.dispatch(
			exporterActions.updateExporter({ exporterModalOpen: false })
		);
	}
	inputHandler(e) {
		this.props.dispatch(
	...
	addVideoToPlaylist(index, playlistId) {
		...
		axios
			...
			.then((res) => {
				console.log(
					`${this.props.chart.songs[index].rank}. ${this.props.chart.songs[index].title} was added to the playlist`
				);
				this.props.dispatch(
					exporterActions.updateExporter({
						exportComplete: this.props.exporter.exportComplete + 1,
					})
				);
			...
			.then((res) => {
				if (res.data.items.length > 0) {
					...
					axios
						...
						.then(() => {
							console.log(
								`Renamed to ${this.props.chart.playlistTitle}`
							);
	...
	export() {
		if (this.props.exporter.exportType === "new") {
			...
			if (regexPlaylist.test(this.props.exporter.playlistId)) {
				this.props.dispatch(
					exporterActions.updateExporter({ exportStatus: "" })
				);
				if (this.props.exporter.existingType === "reset") {
					this.removeVideoFromPlaylist(
						this.props.exporter.playlistId
					);
			...
			<div className="Exporter">
				...
				>
					...
					<div className="section">
						<label>
							<input
								...
								checked={
									this.props.exporter.exportType === "new"
								}
							/>
							새로운 플레이리스트로 보내기 (주의: 제한 넘길시
							24시간 동안 생성 불가)
						</label>
					</div>
					<div className="section">
						<label>
							<input
								...
								onChange={this.inputHandler}
								checked={
									this.props.exporter.exportType ===
									"existing"
								}
							/>
							...
							<div>
								<label>
									<input
										...
										onChange={this.inputHandler}
										checked={
											this.props.exporter.existingType ===
											"reset"
										}
										disabled={
											this.props.exporter.exportType ===
					...
					<ProgressBar
						...
						bsStyle="success"
						label={
							this.props.exporter.exportComplete > 0
								? "Exporting"
								: this.props.exporter.removingVideos
								? "Deleting"
								: ""
						}
						active
						now={
							this.props.exporter.exportComplete ||
							(this.props.exporter.removingVideos ? 100 : 0)
						}
					/>
					{this.props.exporter.exportComplete === 100 ? (
						<div>완료!</div>
					) : (
						<div>{this.props.exporter.exportStatus}</div>
					)}
				</Modal>
