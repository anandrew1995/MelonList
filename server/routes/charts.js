...
router.get("/", (req, res) => {
	...
	switch (chartType) {
		case "month":
			lastUpdateDate =
				(lastUpdateDate.date() === 1 && lastUpdateDate.hour() >= 14) ||
				lastUpdateDate.date() > 1
					? lastUpdateDate.startOf("month").add(14, "hours")
					: lastUpdateDate
							.startOf("month")
							.subtract(1, "months")
							.add(14, "hours");
		...
		case "week":
			lastUpdateDate =
				(lastUpdateDate.day() === 2 && lastUpdateDate.hour() >= 14) ||
				lastUpdateDate.day() > 2
					? lastUpdateDate
							.startOf("week")
							.add(2, "days")
							.add(14, "hours")
		...
		case "day":
			lastUpdateDate =
				lastUpdateDate.hour() >= 14
					? lastUpdateDate.startOf("day").add(14, "hours")
					: lastUpdateDate
							.startOf("day")
							.subtract(1, "days")
							.add(14, "hours");
	...
	Chart.findOne(
		{ classCd, chartType, retrievedDate: lastUpdateDate },
		(err, chart) => {
			if (err) throw err;
			if (!chart) {
